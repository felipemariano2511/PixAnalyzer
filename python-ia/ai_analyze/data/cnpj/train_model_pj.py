...
df['dias_desde_status_date'] = (datetime.now() - pd.to_datetime(df['status_date'], errors='coerce')).dt.days.fillna(0)
df['dias_desde_timestamp'] = (datetime.now() - pd.to_datetime(df['timestamp'], errors='coerce')).dt.days.fillna(0)

df = df.drop(columns=['registration_date', 'status_date', 'timestamp'])

drop_cols = ['id', 'destination_key_value', 'cnpj', 'company_name', 'confidence_score', 'fraud_reasons']
